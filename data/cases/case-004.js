(function () {
  ...
  caseFour.masteryFocus = {
    ...
    transfer: "If Aiden developed the same unilateral absent breath sounds and shock immediately after positive-pressure ventilation but had no visible chest trauma, what would still make you decompress before imaging?",
    transferAnswer: "Positive-pressure ventilation itself can precipitate a tension pneumothorax. Sudden deterioration with unilateral absent air entry, hypoxaemia and obstructive shock is enough to decompress clinically before imaging, then reassess and arrange definitive pleural drainage.",
    transferChecks: [
      "Sudden deterioration after positive pressure",
      "Unilateral absent air entry or asymmetry",
      "Hypoxaemia plus shock physiology",
      "Immediate response after decompression"
    ],
    transferCitationIds: ["qas-decompression-2026", "qld-pccm-chest-2025"]
  };
  ...
  caseFour.hints = [
    ...
    c004Hint(2, "stem", "c004-stem-patient", "motorcycle collision about 20 minutes ago", ["qld-pccm-chest-2025", "qas-chest-2022"], "Reading the mechanism", "Recent motorcycle collision. Should I focus only on the chest?", "Keep the whole trauma in view. A direct chest injury may be obvious, but rapid deceleration can injure several regions. Start with the threat that can kill now, then complete the trauma survey.", "One painful area does not exclude other injuries.", "This is recent high-energy trauma.", "Check whether he is stable before asking for collision details."),
    ...
    c004Hint(7, "stem", "c004-task-explain", "working diagnosis and further management", ["amc-spec-2026", "amc-tips-2024"], "Reading task 2", "Is this another assessment?", "At minute 5, stop collecting detail. Name the working diagnosis, show which findings established it, mention injuries that may coexist and state what happens next.", "When the prompt changes, follow exactly what it now asks.", "Two minutes are reserved for synthesis and ongoing care.", "Prepare a short explanation built from findings and treatment response."),
    ...
    c004Hint(10, "run", "c004-run-first-action", "calls for the trauma and resuscitation teams", ["qld-pccm-chest-2025", "qas-chest-2022"], "Calling for help", "Am I escalating too early?", "Early escalation fits this presentation. Severe breathlessness after major trauma may need simultaneous airway, chest, circulation and procedural support. Early help lets treatment occur without abandoning spinal care or the rest of the survey.", "Calling for help is part of treatment.", "The team is assembling while you approach.", "Introduce yourself briefly and assess whether he can speak."),
    ...
    c004Hint(17, "run", "c004-run-decompression-action", "A rush of air is heard", ["qas-decompression-2026", "qas-chest-2022"], "Reading the immediate response", "The rush of air sounds successful. Am I finished?", "Not yet. Deeper breaths and longer speech support effective pressure release, but the catheter can fail or dislodge. Improvement must be confirmed through ABC reassessment.", "A procedure result is not a substitute for patient response.", "Pressure has been released and Aiden looks better.", "Repeat breathing, oxygenation, circulation and conscious state now."),
    ...
    c004Hint(40, "run", "c004-run-handover-prompt", "concise handover", ["amc-spec-2026", "acsqhc-handover-2026"], "Changing to handover", "How do I fit this into one minute?", "Use four blocks: who he is and what happened; the life threat; what you did and the response; then the device in place and what must happen next.", "Do not restart the case from the greeting.", "The final task is transfer of responsibility.", "Begin with identity, mechanism and the immediate problem."),
    ...
    c004Hint(44, "run", "c004-run-end", "The station is complete", ["amc-tips-2024"], "Leaving the station", "Should I add one more point?", "Stop when the examiner closes the station. Take the useful pattern with you: unstable trauma, treat the reversible threat, reassess, resume, hand over.", "Leave unfinished sentences here.", "This station is finished.", "Take one breath and read the next stem from its first word.")
  ];

  caseFour.essentialHintIds = [
    "c004-hint-04", "c004-hint-06", "c004-hint-07", "c004-hint-08",
    "c004-hint-13", "c004-hint-14", "c004-hint-15", "c004-hint-18",
    "c004-hint-19", "c004-hint-24", "c004-hint-34", "c004-hint-35",
    "c004-hint-40", "c004-hint-42", "c004-hint-43", "c004-hint-44"
  ];

  var c004Journey = {
    "c004-hint-06": { clock: "0 to 5 minutes: assess, treat and reassess." },
    "c004-hint-13": {
      clock: "Danger interrupts the clock: treat this now.",
      say: ["This is a right tension pneumothorax causing obstructive shock. The unstable pattern makes this a clinical diagnosis."],
      deeper: ["Trauma has let air enter the pleural space. Rising pressure prevents the right lung expanding and reduces venous return to the heart, causing severe hypoxaemia, hypotension and poor perfusion."]
    },
    "c004-hint-16": {
      say: ["Use trained technique, a purpose-designed device, an approved local site, secure fixation and immediate reassessment."],
      deeper: ["Approved devices, patient anatomy and local protocols differ, so one insertion site cannot safely be treated as universal."]
    },
    "c004-hint-24": { clock: "Return to task 1: finish the primary survey." },
    "c004-hint-34": { clock: "5 to 7 minutes: explain the diagnosis and further care." },
    "c004-hint-40": {
      clock: "7 to 8 minutes: concise handover.",
      deeper: ["Add only the few observations that explain why treatment was urgent and whether it worked."]
    },
    "c004-hint-44": { clock: "Station closed: one breath, then leave this case here." }
  };

  caseFour.hints.forEach(function (hint) {
    var journey = c004Journey[hint.id];
    if (!journey) return;
