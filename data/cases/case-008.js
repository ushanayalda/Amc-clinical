(function () {
  ...
  caseEight.reasoningCompass = {
    stem: {
      ...
      steps: [
        ...
        { time: "1 to 2 minutes", text: "Fix the switches: interpret results at minute 4, then explain the diagnosis and ongoing treatment at minute 6." }
      ],
    ...
    run: {
      ...
      anchor: "If airway protection, breathing effort or conscious state worsens, interrupt the task, stabilise first, then return to the unfinished task verb."
    }
  };
  ...
  caseEight.hints = [
    ...
    c008Hint("c008-hint-05", "stem", "c008-task-assess", "Assess and manage the patient", ["c008-amc-spec", "c008-amc-tips", "c008-copdx-main"], "Anchoring task one", "Assess and manage. Does that mean I finish assessment before treatment?", "No. Ask for the oxygen device, observations and conscious state early. Correct dangerous treatment, give bronchodilators and call for help while the focused assessment continues.", "The two verbs run together when the patient is unstable.", "Minutes 0 to 4 combine ABCDE, focused cause finding and immediate treatment.", "Keep enough time to interpret the investigation results.", "0 to 4 minutes"),
    c008Hint("c008-hint-06", "stem", "c008-task-interpret", "Interpret the investigation results", ["c008-amc-spec", "c008-amc-tips"], "Anchoring task two", "What does interpret require beyond reading numbers?", "State the acid-base disorder, whether it is acute or chronic, what the oxygen result means on the current device and how those findings change treatment.", "Interpretation must lead to an action.", "At minute 4, stop collecting extra history and explain the result's immediate significance.", "Reserve the last two minutes for Peter's explanation.", "4 to 6 minutes"),
    c008Hint("c008-hint-07", "stem", "c008-task-explain", "ongoing treatment to the patient", ["c008-amc-spec", "c008-amc-tips", "c008-communication"], "Anchoring task three", "How much technical detail belongs in the explanation?", "Give the cause in plain language, explain why oxygen is controlled rather than stopped, what the mask does and what will happen if he worsens. Check one important point back with him.", "Do not turn the explanation into another blood-gas interpretation.", "The final two minutes belong to Peter's concern and the ongoing plan.", "Begin the run and look for the first sign that routine pacing is unsafe.", "6 to 8 minutes"),
    ...
    c008Hint("c008-hint-09", "run", "c008-run-first-action", "resuscitation trolley", ["c008-copdx-main", "c008-amc-tips"], "Preparing safely", "Is bringing emergency equipment excessive?", "No. Severe breathlessness and possible reduced consciousness can deteriorate quickly. Preparation lets assessment continue without waiting if airway or ventilation fails.", "Preparation does not replace assessment.", "Help and equipment are close while you approach.", "Hear Peter's first words before deciding what is happening."),
    ...
    c008Hint("c008-hint-11", "run", "c008-run-abc", "including the oxygen device", ["c008-copdx-oxygen", "c008-anzcor-oxygen"], "Requesting the essentials", "Why name the device, not just the saturation?", "A saturation only makes sense beside the oxygen concentration and flow. Ninety-nine per cent on room air and ninety-nine per cent on high-concentration oxygen mean very different things.", "Always pair oxygen saturation with the device and flow rate.", "You are assessing breathing, circulation and conscious state while identifying treatment already given.", "Use the examiner's findings before changing anything."),
    c008Hint("c008-hint-12", "run", "c008-run-findings", "10 litres per minute through a non-rebreather mask", ["c008-copdx-oxygen"], "Reading current treatment", "High-concentration oxygen is already running. Should I remove it?", "No. Peter needed oxygen, and sudden removal could cause hypoxaemia. Change to a controlled device, prescribe a target and watch him closely while you obtain a blood gas.", "Correct the dose; do not abandon the treatment.", "He is receiving much more oxygen than usually required for COPD.", "Now combine the device with saturation, breathing effort and conscious state."),
    ...
    c008Hint("c008-hint-37", "run", "c008-run-handover", "being investigated", ["c008-copdx-main", "c008-copdx-niv", "c008-communication"], "Optional real-care handover", "If permitted, what is the first thing the receiving team needs?", "The station did not request a handover. After completing assessment, interpretation and explanation, use any permitted time to state severity, current support and trajectory, then give the trigger, initial oxygen exposure, gases, treatment, response and the plan if non-invasive ventilation fails.", "Do not sacrifice a requested task for this handover.", "The next team knows what happened, what has improved and what still threatens Peter.", "Finish and wait for the examiner."),
    c008Hint("c008-hint-38", "run", "c008-run-end", "The station is complete", ["c008-amc-spec", "c008-amc-tips"], "Leaving the station", "Should I keep checking what I forgot?", "No. The case has ended. One breath, then begin the next stem from its own role, symptoms and tasks.", "Do not carry Peter's oxygen target into a different patient.", "Case 8 is finished.", "Start the next case with no assumed diagnosis.")
  ];
})();
