  ...
  caseThirteen.reasoningCompass = {
    stem: {
      ...
      steps: [
        ...
        { time: "1 to 2 minutes", text: "Hold the switches: assess and manage to minute 3, interpret repeat findings to minute 6, explain to minute 7, hand over in the final minute." }
      ],
    ...
    run: {
      ...
      anchor: "If breathing, circulation or conscious state worsens, interrupt the task, repeat ABCDE and escalate; then return to the unfinished task verb."
    }
  };
  caseThirteen.hints = [
    ...
    buildHint13("c013-hint-02", "stem", "c013-stem-patient", "58-year-old man", ["amc-tips-2024", "acsqhc-deterioration-2026"], "Reading age and context", "Does 58 point to one cause?", "No. Age raises the chance of serious infection and cardiopulmonary disease, but the current physiology will decide urgency.", "Do not settle on a cause from age.", "Kieran is 58; stability is still unknown.", "Read the symptom cluster and duration."),
    ...
    buildHint13("c013-hint-05", "stem", "c013-stem-start", "findings and investigation results when requested", ["amc-spec-2026", "amc-tips-2024"], "Planning requests", "What will I only receive if I ask?", "The station is telling you to verbalise a structured examination and purposeful tests. Request ABCDE findings first, then investigations that answer source, organ injury and treatment response.", "Do not wait for findings to appear on their own.", "Your requests control the information flow.", "Prepare the first complete assessment request."),
    ...
    buildHint13("c013-hint-07", "stem", "c013-task-interpret", "initial and repeat findings", ["amc-spec-2026", "amc-tips-2024"], "Anchoring task two", "Why does the task say repeat?", "Because the response to treatment is part of the diagnosis and management. Compare perfusion, lungs and urine after each intervention; do not repeat the original plan automatically.", "A treatment trial needs a measured response.", "Minutes 3 to 6 test adjustment, not memory.", "Look for what improved, what did not and what new harm appeared."),
    ...
    buildHint13("c013-hint-22", "run", "c013-run-history", "how did his breathing and cough change today", ["acsqhc-sepsis-2022", "amc-tips-2024"], "Returning to the history", "Have I lost the history task?", "No. Immediate care is underway, so return to the unfinished question: onset and change today. Anika can answer while Kieran receives treatment.", "After an interruption, return to the exact unfinished task.", "ABCDE and first treatment are complete.", "Clarify respiratory source features and rapid deterioration."),
    buildHint13("c013-hint-23", "run", "c013-run-history-answer", "rusty sputum", ["acsqhc-sepsis-2022"], "Refining the source", "Does rusty sputum prove a bacterium?", "No. It supports lower respiratory infection with the pleuritic pain and focal chest signs, but it does not identify the organism. Influenza exposure keeps viral infection and bacterial coinfection relevant.", "Specific-looking sputum is still not microbiology.", "Pneumonia remains the leading source.", "Check competing sources and recent healthcare exposure."),
    ...
    buildHint13("c013-hint-31", "run", "c013-run-vasopressor", "Stop further rapid boluses", ["qld-sepsis-2025", "acsqhc-sepsis-2022"], "Acting on reassessment", "Am I under-resuscitating him?", "No. You gave measured fluid while it helped and stopped when benefit disappeared and lung congestion emerged. Continue noradrenaline and organ support, then follow perfusion and lactate.", "Safe resuscitation changes with the patient.", "The current problem needs vasopressor support, not another rapid bolus.", "Explain this change clearly when Anika asks."),
    ...
    buildHint13("c013-hint-34", "run", "c013-run-time", "Please explain the diagnosis", ["amc-spec-2026", "amc-tips-2024"], "Recovering at the task switch", "I was still interpreting. What now?", "Stop. The next required verb is explain. Give the source, what sepsis and shock mean, what has been done and why the plan changed.", "Do not keep adding hidden analysis after the prompt.", "Assessment and interpretation are complete.", "Speak to Kieran and Anika in everyday language."),
    ...
    buildHint13("c013-hint-39", "run", "c013-run-end", "The station is complete", ["amc-tips-2024"], "Releasing the station", "Should I keep correcting my answer?", "No. The station has ended. Take one breath and let this case finish before reading the next role and tasks.", "Do not carry the previous emergency into the next stem.", "Case 13 is complete.", "Begin the next case from its own first line." )
  ];
