  ...
  caseFourteen.reasoningCompass = {
    stem: {
      ...
      steps: [
        ...
        { time: "1 to 2 minutes", text: "Fix the switches: history and condition to minute 3, interpretation and management to minute 6, explanation to minute 7, handover to minute 8." }
      ],
    ...
    run: {
      ...
      anchor: "If breathing, circulation or conscious state worsens, interrupt routine questions, repeat ABCDE and escalate; then return to the unfinished task verb."
    }
  };
  caseFourteen.hints = [
    ...
    buildHint14("c014-hint-07", "stem", "c014-task-history-assess", "focused history and assess Celeste's current condition", ["amc-spec-2026", "amc-tips-2024", "acsqhc-deterioration-2026"], "Anchoring task one", "History first, then examination?", "No. The wording joins them. Ask the critical chemotherapy and fever questions while checking airway, breathing, circulation, conscious state and glucose.", "Parallel work protects time and safety.", "The first three minutes combine history and physiology.", "Do not complete a long source checklist before observations."),
    buildHint14("c014-hint-task-interpret", "stem", "c014-task-interpret", "Interpret the information provided and discuss your management", ["amc-spec-2026", "amc-tips-2024"], "Anchoring task two", "What changes at minute three?", "Stop collecting routine history and combine the treatment timing, fever timeline, observations and blood results. Then state transfer, antibiotic and monitoring decisions.", "Interpretation means turning evidence into action.", "Minutes 3 to 6 are for diagnosis, urgency and management.", "Keep the patient explanation for the next task."),
    ...
    buildHint14("c014-hint-08", "stem", "c014-task-handover", "concise clinical handover", ["amc-spec-2026", "acsqhc-communication-2026"], "Anchoring the final minute", "What makes this handover useful?", "State chemotherapy day, highest temperature and antipyretic, rigors, observations, neutrophil count, source findings, transfer status and the required antibiotic time.", "Do not retell every negative symptom.", "The final minute transfers risk and unfinished treatment.", "Keep those facts available as they appear."),
    buildHint14("c014-hint-09", "run", "c014-run-start", "You may begin", ["amc-spec-2026", "amc-tips-2024"], "Starting the encounter", "What do I do first?", "Introduce yourself, hear her priority and begin observations while you talk. The focused history and condition assessment start together.", "Do not spend the opening minute on a complete history.", "You have three minutes for history plus current condition.", "Listen for what changed overnight and what may be hidden by treatment."),
    ...
    buildHint14("c014-hint-20", "run", "c014-run-fever-history-answer", "38.3 at 6 am, then I took paracetamol", ["eviq-neutropenic-fever-2026", "acsqhc-sepsis-2022"], "Restoring the timeline", "Do the clues now earn a leading diagnosis?", "Yes, as a working emergency diagnosis. Fever before paracetamol, recent chemotherapy, expected count fall and current physiological stress make possible neutropenic sepsis the leading concern.", "The diagnosis is provisional; the action is urgent.", "Possible neutropenic sepsis now leads.", "Check possible sources, hydration and resistance or allergy history while arranging escalation."),
    ...
    buildHint14("c014-hint-22", "run", "c014-run-source-history-answer", "hardly drunk today", ["acsqhc-deterioration-2026", "qld-primary-sepsis-2026"], "Reading intake and perfusion", "Is the slow refill only dehydration?", "Poor intake may contribute, but it cannot safely explain rigors and treatment-related immune risk. A measured fluid trial may help perfusion while emergency transfer proceeds.", "Do not replace a dangerous explanation with a partial one.", "There may be both volume depletion and infection.", "Check allergy, recent antibiotics, resistant organisms and the PICC site."),
    buildHint14("c014-hint-23", "run", "c014-run-line-finding", "clean and non-tender", ["eviq-neutropenic-fever-2026"], "Interpreting a normal line site", "Does this exclude a line infection?", "No. It lowers concern for a local exit-site infection, but bloodstream infection can occur without redness or tenderness. Cultures still need to include each lumen when the hospital pathway starts.", "Normal local appearance does not exclude bloodstream infection.", "There is still no clear source.", "Combine timing, fever and physiology and state the leading emergency concern."),
    buildHint14("c014-hint-24", "run", "c014-run-recognition", "make neutropenic sepsis possible", ["eviq-neutropenic-fever-2026", "acsqhc-sepsis-2022", "qld-primary-sepsis-2026"], "Naming the leading concern", "Do the clues now earn the diagnosis?", "Yes, as a working emergency diagnosis. Chemotherapy timing, documented fever, rigors, mucositis, tachycardia and slow refill justify treating this as possible neutropenic sepsis before the count or source is confirmed.", "The diagnosis is provisional; the action is urgent.", "Possible neutropenic sepsis is now the leading concern.", "Interrupt routine history and arrange monitored transfer and oncology contact."),
    buildHint14("c014-hint-25", "run", "c014-run-actions", "Call Triple Zero for a monitored ambulance", ["eviq-neutropenic-fever-2026", "qld-primary-sepsis-2026"], "Escalating from the clinic", "Could Jonah drive her faster?", "No. She has circulation changes and may deteriorate. Monitored ambulance transfer allows reassessment and coordinated handover while the receiving team prepares.", "Transfer is treatment when the clinic cannot provide definitive care.", "Emergency escalation has interrupted the routine sequence appropriately.", "Monitor, obtain access if it causes no delay and confirm the antibiotic plan."),
    ...
    buildHint14("c014-hint-31", "run", "c014-run-time", "Please explain your concerns", ["amc-spec-2026", "amc-tips-2024"], "Recovering at the task switch", "What if I still have management points?", "Stop and follow the verb explain. Tell Celeste and Jonah what the low count means, why one temperature is misleading and what will happen during transfer and admission.", "Do not continue talking to the examiner after the prompt.", "Interpretation and management are complete.", "Use short causal sentences and check that the plan is clear."),
