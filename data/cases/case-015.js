(function () {
  ...
  caseFifteen.masteryFocus = {
    ...
    tasks: [
      "Assess and manage",
      "Interpret and continue management",
      "Explain assessment and plan",
      "Concise clinical handover"
    ],
    transfer: "After a fall, another patient has pelvic pain, tachycardia and cool skin but a normal pressure, normal first haemoglobin and negative eFAST. Which findings would make you act before CT?",
    transferAnswer: "The patient may be in compensated haemorrhagic shock despite a normal pressure, first haemoglobin and eFAST. Mechanism, pelvic pain, tachycardia, cool skin, delayed perfusion or worsening lactate justify a pelvic binder, major-haemorrhage activation and blood-based resuscitation before CT; CT is for a patient stable enough to leave resuscitation.",
    transferChecks: [
      "Perfusion and vital-sign trend",
      "Pelvic mechanism, pain and instability",
      "Recognise eFAST cannot exclude retroperitoneal bleeding",
      "Response to binder, blood and resuscitation"
    ],
    transferCitationIds: ["aci-trauma-shock-2026", "qas-pelvic-injury-2021"]
  };
  caseFifteen.sources = [
  ...
  caseFifteen.reasoningCompass = {
    stem: {
      ...
      steps: [
        ...
        { time: "1 to 2 minutes", text: "Set the task change points: assess and manage to minute 3, interpret and continue management to minute 6, explain to minute 7, hand over to minute 8." }
      ],
    ...
    run: {
      ...
      anchor: "If airway, breathing, circulation or conscious state worsens, interrupt the sequence, repeat ABCDE and move to definitive control; then return to the question or action you paused."
    }
  };
  caseFifteen.hints = [
    ...
    buildHint15("c015-hint-06", "stem", "c015-task-assess", "Assess Otis and discuss your initial management", ["amc-spec-2026", "amc-tips-2024"], "Anchoring task one", "What counts as assess here?", "External bleeding, ABCDE, exposure with warming, focused trauma examination, monitoring and immediate management. The mechanism history comes around those priorities.", "The emergency assessment leads; the history fits around it.", "The first three minutes require assessment plus action.", "Prepare the trauma response, binder, access and blood support decisions."),
    buildHint15("c015-hint-07", "stem", "c015-task-interpret", "ongoing management", ["amc-spec-2026", "amc-tips-2024"], "Anchoring task two", "Ongoing management: what am I supposed to add?", "Show how each new finding changes the plan. State what you would continue, stop, add or escalate, and explain which investigation or treatment is safe at that point.", "Management should change when stability or evidence changes.", "Task two is interpretation followed by an updated plan.", "Keep the first assessment and later findings separate so you can compare them.", "Minutes 3 to 6: interpret new information and update management."),
    buildHint15("c015-hint-task-explain", "stem", "c015-task-explain", "Explain your assessment and management plan", ["amc-spec-2026", "amc-tips-2024"], "Anchoring task three", "What belongs in the explanation minute?", "Speak to Otis about the suspected injury, hidden bleeding risk, treatment already underway and why the next investigation or procedure depends on stability.", "Use patient language here; clinical shorthand belongs in the handover.", "Minute 6 to 7 is explanation.", "Keep minute 7 to 8 for the senior handover."),
    buildHint15("c015-hint-08", "stem", "c015-task-handover", "concise clinical handover", ["amc-spec-2026", "acsqhc-communication-2026"], "Anchoring the final minute", "One minute. What belongs in the handover?", "Give Otis's identity, the collision and symptoms, the most important findings available by then, your working diagnosis, treatment and response, and the immediate next action.", "The senior clinician needs the current problem and unfinished work.", "The explanation and handover are separate tasks.", "Keep a brief note of key changes as the case develops.", "Minute 7 to 8: concise senior handover."),
    ...
    buildHint15("c015-hint-14", "run", "c015-run-primary-findings", "blood pressure is 118/76", ["acsqhc-deterioration-2026"], "Avoiding false reassurance", "Normal pressure. Is shock excluded?", "A healthy adult can maintain pressure through vasoconstriction and tachycardia. Pulse 118 beside a normal pressure may be early compensation.", "Pressure may fall late.", "Circulation may be impaired despite a normal number.", "Keep reading for refill, skin, temperature and injury findings."),
    buildHint15("c015-hint-15", "run", "c015-run-primary-findings", "pale, cool", ["aci-trauma-shock-2026", "nba-critical-bleeding-2025"], "Combining circulation findings", "Is there enough evidence to name a leading diagnosis?", "Yes. Tachycardia, delayed refill, light-headedness, cool pallor and pelvic pain support compensated haemorrhagic shock from occult trauma bleeding until proved otherwise.", "The diagnosis comes from perfusion and mechanism, not hypotension alone.", "Hidden pelvic or abdominal blood loss is now the leading concern.", "Activate trauma and major haemorrhage responses immediately."),
    ...
    buildHint15("c015-hint-20", "run", "c015-run-actions", "major haemorrhage protocol are activated", ["nba-critical-bleeding-2025", "aci-trauma-shock-2026"], "Mobilising resources", "Is this too early before imaging?", "Clinical compensated shock with a possible pelvic source justifies preparation. The protocol can be adjusted if later evidence changes the concern.", "Early activation can be stepped down; delayed blood support is harder to recover.", "Binder, warming, access and blood-bank support are underway.", "Return to mechanism and AMPLE questions while resuscitation continues."),
    ...
    buildHint15("c015-hint-33", "run", "c015-run-time", "Please explain your assessment", ["amc-spec-2026", "amc-tips-2024"], "Giving the full explanation", "I already answered him. Do I repeat?", "Now bring the explanation together in one minute: pelvic injury, hidden bleeding, early misleading numbers, treatment given and why CT is unsafe during instability.", "The earlier answer addressed one concern; this prompt asks for the whole plan.", "Assessment and ongoing management are complete.", "Use a short causal sequence, then stop for handover."),
    ...
    buildHint15("c015-hint-37", "run", "c015-run-end", "The station is complete", ["amc-tips-2024"], "Letting the station finish", "Should I keep thinking of missed trauma injuries?", "This station has ended. Take one breath and begin the next stem from its own mechanism, symptoms and tasks.", "Leave the pelvic injury here.", "Case 15 is complete.", "Read the next role and tasks afresh." )
  ];
  caseFifteen.hints.forEach(function (hint, index) { hint.id = "c015-hint-" + String(index + 1).padStart(2, "0"); });
  caseFifteen.essentialHintIds = [
    "c015-hint-03", "c015-hint-06", "c015-hint-07", "c015-hint-08",
    "c015-hint-09", "c015-hint-13", "c015-hint-16", "c015-hint-20",
    "c015-hint-21", "c015-hint-25", "c015-hint-26", "c015-hint-30",
    "c015-hint-31", "c015-hint-34", "c015-hint-37", "c015-hint-39"
  ];
  var clockByHint15 = {
    "c015-hint-06": "Minutes 0 to 3: ABCDE, haemorrhage control and initial trauma management come before a long history.",
    "c015-hint-07": "Minutes 3 to 6: interpret new information and update management.",
    "c015-hint-08": "Minute 6 to 7: explain the assessment and plan to Otis.",
    "c015-hint-09": "Minute 7 to 8: concise senior handover.",
    "c015-hint-30": "Deterioration interrupts the sequence now: repeat ABCDE and move directly towards bleeding control.",
    "c015-hint-34": "Minute 6: explain the suspected injury, hidden bleeding and why the destination changed.",
    "c015-hint-37": "Final minute: hand over mechanism, trajectory, treatment and the direct destination for control.",
    "c015-hint-39": "Time is complete. One breath, then begin the next stem from its first line."
  };
  var deeperByHint15 = {
    "c015-hint-20": ["A binder works best across the greater trochanters because that is where it can reduce pelvic volume and stabilise the ring. At the waist, it sits too high."],
    "c015-hint-26": ["Early whole-blood loss removes plasma and red cells together. The first haemoglobin concentration can therefore remain normal until fluid shifts or resuscitation change the concentration."],
    "c015-hint-27": ["The pelvis and retroperitoneum can hold a large volume of blood without free intraperitoneal fluid, so eFAST may stay negative."]
  };
  caseFifteen.hints.forEach(function (hint) {
    if (clockByHint15[hint.id]) hint.clock = clockByHint15[hint.id];
    if (deeperByHint15[hint.id]) hint.deeper = deeperByHint15[hint.id];
  });
})();
