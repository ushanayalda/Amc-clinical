(function () {
  ...
  caseEighteen.masteryFocus = {
    ...
    transfer: "If Fern instead had chronic liver disease, ascites and previous oesophageal banding but no recent anti-inflammatory medicine, which early treatment choices would change before the camera test?",
    transferAnswer: "The new history makes a variceal source much more likely than an NSAID-related ulcer. Alongside careful resuscitation, start a vasoactive drug and antibiotic prophylaxis according to local protocol, then arrange endoscopy within 12 hours once haemodynamically resuscitated.",
    transferChecks: [
      "Chronic liver disease and previous varices",
      "Airway, perfusion and ongoing haematemesis",
      "Vasoactive therapy plus antibiotic prophylaxis",
      "Endoscopy timing and early TIPS risk assessment"
    ],
  ...
  caseEighteen.hints = [
    ...
    buildHint18("c018-hint-02", "stem", "c018-stem-patient", "63-year-old woman", ["amc-tips-2024", "nba-critical-bleeding-2025"], "Reading age", "Does her age tell me the cause?", "Her age increases concern for significant bleeding and comorbidity, but the source and severity still have to come from the history, observations and examination.", "Age raises concern, but it does not identify the cause.", "Fern may have less reserve than a younger adult.", "Now look at exactly what she vomited and passed."),
    ...
    buildHint18("c018-hint-18", "run", "c018-run-primary-findings", "Pulse is 116, blood pressure 104/66 and capillary refill three seconds", ["acsqhc-deterioration-2026", "nba-critical-bleeding-2025"], "Reading perfusion", "The pressure is above 100. Is that reassuring?", "Tachycardia, delayed refill, pallor and cool skin show impaired perfusion even before severe hypotension appears.", "Use the whole circulation examination, not one pressure value.", "Fern is alert but physiologically compromised.", "Check the abdomen for a complication or another clue to source."),
    ...
    buildHint18("c018-hint-20", "run", "c018-run-access", "two large-bore intravenous cannulas", ["nba-critical-bleeding-2025"], "Acting on poor perfusion", "Why call teams and prepare blood before the haemoglobin returns?", "Her symptoms and circulation already support significant blood loss. Large access, warming, crossmatched blood preparation, small reassessed crystalloid boluses and urine monitoring can start now.", "Her physiology already supports treatment; the laboratory result can follow.", "Early support and senior involvement are underway.", "Keep the crystalloid measured and reassess after each intervention."),
    buildHint18("c018-hint-21", "run", "c018-run-access-action", "warmed fluid is commenced while blood is prepared", ["nba-critical-bleeding-2025", "acsqhc-deterioration-2026"], "Using parallel action", "Should I stop asking questions until every intervention is finished?", "The team can establish monitoring and access while you continue a short history, provided you watch Fern and interrupt if she worsens.", "Parallel care saves time only when reassessment continues.", "Initial circulation support has begun.", "Clarify the appearance, amount and consequences of the bleeding."),
    ...
    buildHint18("c018-hint-39", "run", "c018-run-scope-result", "pressure improves to 102/64", ["nba-critical-bleeding-2025", "acsqhc-deterioration-2026"], "Using the response", "Does one improved pressure mean the danger has passed?", "It shows an initial response to blood and makes the procedure safer, but the recent shock and active bleeding still require definitive control and close monitoring.", "The pressure has improved, but recent shock and active bleeding still require endoscopic control and close monitoring.", "Circulation has improved enough to proceed with urgent source assessment.", "Now let the camera finding define the lesion."),
    ...
    buildHint18("c018-hint-49", "run", "c018-run-handover-prompt", "concise handover to the senior clinician", ["amc-spec-2026", "acsqhc-communication-2026"], "Recovering the final task", "What order keeps this handover useful?", "Use four blocks: presentation and deterioration; current airway and circulation; treatment and response; then bleeding control and the immediate rebleeding plan.", "Use only the history that explains current risk and treatment.", "The receiving clinician needs current risk and action in one minute.", "Give the lowest point, the response and what is underway now."),
    ...
    buildHint18("c018-hint-51", "run", "c018-run-end", "The station is complete", ["amc-tips-2024"], "Letting the station finish", "Do I need to add one more detail?", "The station has ended. Pause briefly, then start the next case from its own stem and tasks.", "The handover is finished.", "Case 18 is complete.", "Read the next stem from the first line.")
  ];
  caseEighteen.hints.forEach(function (hint, index) { hint.id = "c018-hint-" + String(index + 1).padStart(2, "0"); });
  caseEighteen.essentialHintIds = [
    "c018-hint-03", "c018-hint-05", "c018-hint-08", "c018-hint-09",
    "c018-hint-10", "c018-hint-11", "c018-hint-18", "c018-hint-20",
    "c018-hint-23",
    "c018-hint-31", "c018-hint-33", "c018-hint-34", "c018-hint-39",
    "c018-hint-46", "c018-hint-49", "c018-hint-51"
  ];
  var clockByHint18 = {
    "c018-hint-08": "Minutes 0 to 3: focused bleeding history and current-condition assessment run together.",
    "c018-hint-21": "Initial treatment is running. Continue only the short questions that change severity, source or treatment.",
    "c018-hint-33": "Fresh haematemesis with deterioration interrupts every other task. Return to ABCDE and major-haemorrhage care now.",
    "c018-hint-39": "Before minute 6: reassess the response, then move to source control while she is supported.",
    "c018-hint-46": "Minute 6: turn to Fern and explain the bleed, treatment, confirmed source and remaining risk.",
    "c018-hint-49": "Final minute: transfer the deterioration, response, endoscopic finding and rebleeding plan.",
    "c018-hint-51": "Time is complete. One breath, then begin the next stem from its first line."
  };
  var deeperByHint18 = {
    "c018-hint-23": ["Blood exposed to gastric acid darkens and clots into a coffee-ground appearance. It suggests blood has spent some time in the upper digestive tract, but it does not identify the lesion."],
    "c018-hint-36": ["A stable haemoglobin threshold applies after circulation has been restored. During active shock, current loss and organ perfusion guide blood replacement."],
    "c018-hint-40": ["A visible vessel in an ulcer is an exposed artery beneath the ulcer base. Even when bleeding slows, that artery carries substantial rebleeding risk and needs endoscopic haemostasis."]
  };
  caseEighteen.hints.forEach(function (hint) {
    if (clockByHint18[hint.id]) hint.clock = clockByHint18[hint.id];
    if (deeperByHint18[hint.id]) hint.deeper = deeperByHint18[hint.id];
  });
})();
