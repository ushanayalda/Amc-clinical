...
(function () {
  ...
  caseNineteen.masteryFocus = {
    ...
    transfer: "If the same bleeding presentation remained alert and stabilised after initial blood support, which findings would determine airway intervention, procedure timing and the next escalation plan?",
    transferAnswer: "Being alert, protecting the airway and stabilising argues against routine prophylactic intubation. Intubate for ongoing haematemesis, encephalopathy, agitation or loss of airway control; proceed to endoscopy within 12 hours after resuscitation, then use active bleeding, liver severity and rebleeding to decide pre-emptive or rescue TIPS.",
    transferChecks: [
      "Ongoing haematemesis and airway protection",
      "Haemodynamic response to resuscitation",
      "Endoscopy within the post-resuscitation window",
      "Child–Pugh/MELD, active bleeding and recurrence"
    ],
    transferCitationIds: ["esge-variceal-2022", "baveno-vii-2022"]
  };
  ...
  caseNineteen.reasoningCompass = {
    stem: {
      ...
      steps: [
        { time: "0 to 30 seconds", text: "Vomiting blood twice means a potentially significant upper gastrointestinal bleed. Keep the exact source uncertain. Prepare to establish current bleeding, amount, melaena, presyncope, pain and immediate ABCDE risk." },
  ...
  caseNineteen.hints = [
    ...
    buildHint19("c019-hint-15", "run", "c019-run-bleeding-answer", "bright red with clots, nearly a cup each time", ["acg-upper-gi-2021", "acsqhc-deterioration-2026"], "Hearing significant fresh bleeding", "Does the patient's estimate have to be exact?", "Repeated bright blood with clots and a substantial estimated volume is enough to treat this as significant bleeding while objective physiology is obtained.", "An exact conversion from a cup will not change the immediate response.", "The bleeding is more concerning than the stem alone showed.", "Add melaena and presyncope, then move promptly to ABCDE."),
    ...
    buildHint19("c019-hint-20", "run", "c019-run-recognition", "significant upper gastrointestinal bleed with impaired circulation", ["acsqhc-deterioration-2026", "nba-critical-bleeding-2025"], "Naming the current problem", "Is this enough interpretation before the cause is known?", "You can now say this is a significant upper gastrointestinal bleed, that circulation is affected and that chronic liver disease is present. The exact bleeding point still needs endoscopy.", "State what the evidence proves; the exact source is not known yet.", "Sami needs emergency, anaesthetic, intensive-care and gastrointestinal support.", "Start access, tests, warming and blood preparation."),
    ...
    buildHint19("c019-hint-27", "run", "c019-run-results", "Haemoglobin is 73 g/L", ["nba-critical-bleeding-2025", "esge-variceal-2022"], "Reading severity from the results", "Is 73 the only reason to transfuse?", "Sami is actively bleeding with impaired circulation and lactate 5.1. Blood treatment follows the whole clinical picture; another haemoglobin should not delay it.", "A laboratory threshold does not replace shock assessment.", "The results support major blood loss and poor perfusion.", "Activate the local critical-bleeding protocol and reassess repeatedly."),
    buildHint19("c019-hint-28", "run", "c019-run-results", "INR 2.0 and fibrinogen 1.3 g/L", ["nba-critical-bleeding-2025", "baveno-vii-2022"], "Reading haemostasis carefully", "Does the INR tell me to give plasma automatically?", "Cirrhotic INR does not describe the full balance of clotting and bleeding. Use active bleeding, fibrinogen, platelets, local protocol and specialist or viscoelastic guidance rather than correcting a number alone.", "Treat haemostasis as a clinical problem, not an isolated INR.", "Critical bleeding and impaired liver reserve coexist.", "Follow the protocol while avoiding routine products without an indication."),
    ...
    buildHint19("c019-hint-31", "run", "c019-run-variceal-plan", "Variceal bleeding is likely", ["esge-variceal-2022", "baveno-vii-2022"], "Starting treatment before proof", "Can treatment begin before the camera test?", "Yes. The cirrhosis, previous banding and current bleed make the suspicion strong enough for vasoactive treatment and antibiotic prophylaxis now, while endoscopy confirms and treats the source.", "The current evidence supports time-sensitive treatment before confirmation.", "Recurrent variceal bleeding is the leading diagnosis, not yet confirmed.", "Continue resuscitation and arrange timely endoscopy."),
    buildHint19("c019-hint-32", "run", "c019-run-variceal-plan", "within 12 hours of presentation after haemodynamic resuscitation", ["esge-variceal-2022", "baveno-vii-2022"], "Timing endoscopy safely", "Does within 12 hours mean before stabilisation?", "Resuscitation, airway assessment, vasoactive treatment and antibiotics start first. Endoscopy then provides definitive diagnosis and haemostasis within the recommended window.", "Arrange endoscopy within 12 hours, but stabilise airway and circulation first.", "Cause-directed treatment and procedural preparation are underway.", "Confirm the medicines have started and answer Sami's uncertainty."),
    ...
    buildHint19("c019-hint-45", "run", "c019-run-time", "Please provide your handover", ["amc-spec-2026", "acsqhc-communication-2026"], "Changing to handover", "What leads the final minute?", "Start with cirrhosis and major haematemesis, then the worst airway and circulation findings, critical-bleeding treatment, endoscopic diagnosis and control, current response and the pre-emptive and rescue plans.", "The audience and purpose have changed.", "Responsibility is moving to the senior clinician.", "Use the same order: current risk, response, then the action still required."),
    buildHint19("c019-hint-46", "run", "c019-run-handover", "being assessed for pre-emptive TIPS", ["acsqhc-communication-2026", "esge-variceal-2022", "baveno-vii-2022"], "Closing the clinical transfer", "What makes this handover complete?", "It includes presentation, relevant background, deterioration, airway intervention, resuscitation, confirmed source, endoscopic control, current observations and the plan if bleeding recurs.", "The receiving team needs the next risk, not every normal finding.", "The full course and immediate escalation plan have been transferred.", "Stop when the examiner ends the station."),
    buildHint19("c019-hint-47", "run", "c019-run-end", "The station is complete", ["amc-tips-2024"], "Letting the station finish", "Should I add one more management point?", "The station has ended. Take one breath and begin the next case from its own stem, tasks and evidence.", "The handover is finished.", "Case 19 is complete.", "Start the next reading period at its first line.")
  ];
  caseNineteen.hints.forEach(function (hint, index) { hint.id = "c019-hint-" + String(index + 1).padStart(2, "0"); });
  caseNineteen.essentialHintIds = [
    "c019-hint-03", "c019-hint-06", "c019-hint-07", "c019-hint-08",
    "c019-hint-09", "c019-hint-15", "c019-hint-17", "c019-hint-18",
    "c019-hint-25", "c019-hint-33", "c019-hint-38", "c019-hint-39",
    "c019-hint-42", "c019-hint-46", "c019-hint-48", "c019-hint-50"
  ];
  var clockByHint19 = {
    "c019-hint-06": "Minutes 0 to 2: establish current bleeding, perfusion symptoms and the background that could change the source.",
    "c019-hint-17": "At minute 2, or sooner if he worsens: stop the history and obtain full ABCDE findings.",
    "c019-hint-38": "Loss of airway protection interrupts every other task. Suction, controlled airway support and resuscitation come first.",
    "c019-hint-42": "Before minute 6: use endoscopy to confirm and control the source, then state the next rebleeding plan.",
    "c019-hint-48": "Final minute: transfer the bleed, deterioration, airway, resuscitation, endoscopic control and TIPS plan.",
    "c019-hint-50": "Time is complete. One breath, then begin the next stem from its first line."
  };
  var deeperByHint19 = {
    "c019-hint-32": ["Cirrhosis alters both clot-forming and clot-limiting factors. INR measures only part of that balance, so correcting the number alone may not improve haemostasis."],
    "c019-hint-37": ["Portal hypertension raises pressure in veins around the lower oesophagus: dilated thin-walled varices → rupture → rapid haematemesis."],
    "c019-hint-44": ["TIPS creates a channel between portal and hepatic venous flow. Lower portal pressure reduces the force driving recurrent variceal bleeding."]
  };
  caseNineteen.hints.forEach(function (hint) {
    if (clockByHint19[hint.id]) hint.clock = clockByHint19[hint.id];
    if (deeperByHint19[hint.id]) hint.deeper = deeperByHint19[hint.id];
  });
})();
