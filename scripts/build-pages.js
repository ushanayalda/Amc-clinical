...
function buildShell(manifest) {
  ...
  html = html.replace(/content="natural-consultant-v3"/g, `content="${buildId}"`);
  html = html.replace(/window\.__BUILD_ID__ = "natural-consultant-v3";/g, `window.__BUILD_ID__ = ${JSON.stringify(buildId)};`);

  manifest.forEach((asset) => {
    html = html.split(asset.source).join(asset.url);
  });
  html = html.split("?v=natural-consultant-v3").join("");
  return html;
}
