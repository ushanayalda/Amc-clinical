...
function buildShell(manifest) {
  ...
  html = html.replace(/content="reasoning-journey-v2"/g, `content="${buildId}"`);
  html = html.replace(/window\.__BUILD_ID__ = "reasoning-journey-v2";/g, `window.__BUILD_ID__ = ${JSON.stringify(buildId)};`);

  manifest.forEach((asset) => {
    html = html.split(asset.source).join(asset.url);
  });
  html = html.split("?v=reasoning-journey-v2").join("");
  return html;
}
